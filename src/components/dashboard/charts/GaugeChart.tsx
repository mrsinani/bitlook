...
const GaugeChart: React.FC<GaugeChartProps> = ({ 
  ...
  const currentSegment = segments.find(
    segment => percentage >= segment.range[0] && percentage <= segment.range[1]
  );
          ...
          {segments.map((segment, index) => {
            ...
            return (
              <div 
                ...
                style={{
                  clipPath: `polygon(50% 50%, 50% 0%, ${50 + 50 * Math.cos(endAngle * Math.PI / 180)}% ${50 + 50 * Math.sin(endAngle * Math.PI / 180)}%, ${50 + 50 * Math.cos(startAngle * Math.PI / 180)}% ${50 + 50 * Math.sin(startAngle * Math.PI / 180)}%)`
                }}
              ></div>
            );
          })}
          
          <div className="absolute w-3 h-20 bg-white left-1/2 bottom-0 origin-bottom transform -translate-x-1/2" 
            style={{ transform: `translateX(-50%) rotate(${rotationDegree}deg)` }}>
          </div>
