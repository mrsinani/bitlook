...
const HalvingCountdown = ({
  ...
  const { data, loading, error, refetch, isFromCache, timeRemaining } =
    useBitcoinHalving(refreshInterval);
  ...
  const getFormattedHalvingDate = () => {
    try {
      return data?.estimatedHalvingDate
        ? formatTime(data.estimatedHalvingDate)
        : "N/A";
    } catch (error) {
    ...
    <Card className="w-full h-full flex flex-col">
      ...
      <CardContent className="pb-3 flex-grow">
        {loading && !data ? (
          ...
          <div className="flex flex-col gap-5 items-center justify-center h-full py-3">
            ...
            <div className="w-full text-center space-y-2">
              <div className="text-sm">
                ...
                <span className="text-foreground">{getBlocksRemaining()}</span>
              </div>
              <div className="text-sm">
                <span className="font-medium">Expected Date: </span>
                <span className="text-foreground">
                  {getFormattedHalvingDate()}
                </span>
              </div>
              <div className="text-sm">
                <span className="font-medium">Reward Change: </span>
                <span className="text-foreground">{getRewardInfo()}</span>
              </div>
            </div>
