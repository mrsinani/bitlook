...
import { RefreshCw, DatabaseIcon, AlertCircle, AlertTriangle, Zap, BarChart3 } from 'lucide-react';
...
const LightningStats = ({ refreshInterval = 300000 }: LightningStatsProps) => {
  const { data, loading, error, refetch, isFromCache } = useLightningStats(refreshInterval);
  ...
  const getTotalCapacity = () => {
    try {
      return data?.totalCapacity?.btc ? formatBitcoin(data.totalCapacity.btc) : 'N/A';
    } catch (error) {
  ...
  const getAvgChannelSize = () => {
    try {
      ...
      return data.avgChannelSize.btc.toLocaleString(undefined, { 
          ...
          <div className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              <div className="space-y-1">
                ...
                <div className="text-xl font-bold text-foreground">{getNodeCount()}</div>
              </div>
              <div className="space-y-1">
                <div className="text-xs font-medium text-muted-foreground">Channels</div>
                <div className="text-xl font-bold text-foreground">{getChannelCount()}</div>
              </div>
            </div>
            ...
            <div className="pt-2 grid grid-cols-2 gap-4 text-sm">
              <div className="space-y-1">
                <div className="text-xs font-medium text-muted-foreground">Avg. Channel Size</div>
                <div className="text-foreground">{getAvgChannelSize()}</div>
              </div>
              <div className="space-y-1">
                <div className="text-xs font-medium text-muted-foreground">Channels per Node</div>
