    ...
    news: [
      {
        ...
        description: "More countries and institutions are exploring Bitcoin as both a store of value and a payment system...",
      ...
      },
      {
        ...
        description: "The Bitcoin network has adjusted its mining difficulty upward, reflecting increased hash power...",
      ...
      },
      {
        ...
        description: "The annual Bitcoin conference is expected to draw record crowds as interest in cryptocurrency grows...",
...
// Bitcoin price API
export const fetchBitcoinPrice = async (forceRefresh = false): Promise<{
  price: number;
  ...
  try {
    ...
    const response = await axios.get<BitcoinPriceResponse>(`${API_BASE_URL}/api/bitcoin-price`);
    ...
    if (cachedData) {
      console.log('Returning cached Bitcoin price data after error', cachedData);
      return {
        ...cachedData,
        fromCache: true,
...
export const fetchBitcoinMarketData = async (forceRefresh = false): Promise<{
  marketCap: number;
  ...
  try {
    ...
    const response = await axios.get<BitcoinMarketDataResponse>(`${API_BASE_URL}/api/bitcoin-market-data`);
    ...
    const transformedData = {
      ...
      formattedVolume: `$${((response.data.total_volume || 0) / 1e9).toFixed(2)}B`,
    ...
    if (axios.isAxiosError(error)) {
      if (error.response?.status === 429) {
        ...
        throw new Error('Server error. The CoinGecko API may be temporarily unavailable.');
...
// Bitcoin supply API
export const fetchBitcoinSupply = async (forceRefresh = false): Promise<{
  circulatingSupply: number;
  ...
  try {
    ...
    // Fetch fresh data from API
    const response = await axios.get<BitcoinSupplyResponse>(`${API_BASE_URL}/api/bitcoin-supply`);
...
export const fetchBlockchainHeight = async (forceRefresh = false): Promise<{
  height: number;
  ...
  try {
    ...
    // Fetch fresh data from API
    const response = await axios.get<BlockchainHeightResponse>(`${API_BASE_URL}/api/blockchain-height`);
    ...
    // If we have cached data, return it even if expired during an error
    const cachedData = getCachedData<any>(CACHE_KEYS.BLOCKCHAIN_HEIGHT, Infinity);
    if (cachedData) {
      return {
        ...cachedData,
        fromCache: true
...
// Bitcoin fees API
export const fetchBitcoinFees = async (forceRefresh = false): Promise<{
  fastestFee: number;
  halfHourFee: number;
  hourFee: number;
  economyFee: number;
  ...
  try {
    ...
    const response = await axios.get<BitcoinFeesResponse>(`${API_BASE_URL}/api/bitcoin-fees`);
...
// Bitcoin TPS API
export const fetchBitcoinTPS = async (forceRefresh = false): Promise<{
  tps: number;
  timeSpanSeconds: number;
  blocksAnalyzed: number;
  totalTransactions: number;
  ...
  try {
    ...
    const response = await axios.get<BitcoinTPSResponse>(`${API_BASE_URL}/api/bitcoin-tps`);
...
// Bitcoin Halving API
export const fetchBitcoinHalving = async (forceRefresh = false): Promise<{
  currentHeight: number;
  nextHalvingBlock: number;
  blocksRemaining: number;
  estimatedDaysRemaining: number;
  ...
  try {
    ...
    const response = await axios.get<BitcoinHalvingResponse>(`${API_BASE_URL}/api/bitcoin-halving`);
    ...
    if (cachedData) {
      console.log('Returning cached Bitcoin halving data after error', cachedData);
      return {
        ...cachedData,
        fromCache: true,
        lastUpdated: cachedData.lastUpdated || new Date()
...
// Lightning stats API
export const fetchLightningStats = async (forceRefresh = false): Promise<{
  nodeCount: number;
  channelCount: number;
  totalCapacity: {
    btc: number;
  ...
  try {
    ...
    const response = await axios.get<LightningStatsResponse>(`${API_BASE_URL}/api/lightning-stats`);
    
    // Calculate avgChannelsPerNode if not provided by API
    const avgChannelsPerNode = response.data.avgChannelsPerNode || 
      (response.data.nodeCount && response.data.channelCount ? 
        response.data.channelCount / response.data.nodeCount : 0);
    ...
    if (cachedData) {
      console.log('Returning cached Lightning Network stats after error', cachedData);
      return {
        ...cachedData,
        fromCache: true,
        lastUpdated: cachedData.lastUpdated || new Date()
    ...
    console.log('No cache available, returning fallback Lightning Network stats');
...
export const fetchFearGreedIndex = async (forceRefresh = false): Promise<{
  value: number;
  valueClassification: string;
  timestamp: number;
  timeUntilUpdate: number;
  ...
  try {
    ...
    const response = await axios.get<FearGreedIndexResponse>(`${API_BASE_URL}/api/fear-greed-index`);
    ...
    if (cachedData) {
      console.log('Returning cached Fear & Greed Index after error', cachedData);
      return {
        ...cachedData,
        fromCache: true,
        lastUpdated: cachedData.lastUpdated || new Date()
...
// Funding Rate API
export const fetchFundingRate = async (forceRefresh = false): Promise<{
  currentRate: number;
  predictedRate: number;
  sentiment: string;
  markPrice: number;
  ...
  try {
    ...
    const response = await axios.get<FundingRateResponse>(`${API_BASE_URL}/api/funding-rate`);
...
export const fetchMempoolStats = async (forceRefresh = false): Promise<{
  count: number;
  vsize: number;
  totalFee: number;
  ...
  try {
    ...
    const response = await axios.get<MempoolStatsResponse>(`${API_BASE_URL}/api/mempool-stats`);
...
// Bitcoin News API
export const fetchBitcoinNews = async (forceRefresh = false): Promise<{
  news: Array<{
    title: string;
    url: string;
    description: string;
  ...
  try {
    ...
    const response = await axios.get<BitcoinNewsResponse>(`${API_BASE_URL}/api/bitcoin-news`);
    ...
    const processedNews = response.data.news.map(article => {
      ...
      // Positive keywords
      const positiveTerms = ['surge', 'rise', 'gain', 'bull', 'upward', 'grow', 'increase', 'high', 'record', 'rally', 'positive', 'adoption'];
      // Negative keywords  
      const negativeTerms = ['drop', 'fall', 'declin', 'crash', 'bear', 'down', 'decrease', 'low', 'concern', 'warn', 'risk', 'ban', 'regulat'];
